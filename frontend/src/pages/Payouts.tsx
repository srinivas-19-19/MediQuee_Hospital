import { Calendar as CalendarIcon, Stethoscope, Video, Home, FlaskConical, TestTube, IndianRupee, TrendingUp } from "lucide-react"
...
export function Payouts() {
  // Payout trend data — comes from backend analytics. Empty until connected.
  const payoutData: { name: string; value: number }[] = [];

  // Transaction records come from the backend. Empty until connected.
  const transactions: {
    id: string; service: string; type: string; icon: LucideIcon;
    date: string; amount: string; status: string; statusColor: string;
  }[] = [];
    ...
    <div className="flex flex-col bg-background min-h-full pb-6">
      ...
      <div className="flex flex-col gap-5 px-4 pt-4">
        ...
        <button className="flex items-center gap-2 bg-white rounded-xl px-3.5 py-2.5 shadow-sm border border-gray-200/60 w-max interactive-element active:scale-95 transition-transform">
          ...
          <span className="text-[13px] font-semibold text-[#172033]">Select date range</span>
        </button>
          ...
          <div className="bg-white rounded-2xl border border-gray-200/60 shadow-[0_1px_2px_rgba(0,0,0,0.02)] overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between p-3.5 border-b border-gray-100 last:border-0 interactive-element active:bg-gray-50/50">
              ...
              <div className="flex flex-col items-end">
                <span className="text-[14px] font-bold text-[#172033]">—</span>
              </div>
            </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white rounded-2xl p-4 shadow-[0_1px_2px_rgba(0,0,0,0.02)] border border-gray-200/60 flex flex-col gap-1">
              ...
              <span className="text-[20px] font-bold text-[#172033]">—</span>
            </div>
            <div className="bg-white rounded-2xl p-4 shadow-[0_1px_2px_rgba(0,0,0,0.02)] border border-gray-200/60 flex flex-col gap-1">
          ...
          <div className="h-[140px] w-full mt-2 -ml-2">
            {payoutData.length > 0 ? (
              ...
              <div className="h-full w-full flex items-center justify-center ml-2">
                <span className="text-[13px] font-medium text-[#98A2B3]">Payout data unavailable</span>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200/60 shadow-[0_1px_2px_rgba(0,0,0,0.02)] overflow-hidden">
            {transactions.length === 0 ? (
              <EmptyState
                icon={IndianRupee}
                title="No Payouts"
                description="Recent payout transactions will appear here once available."
              />
