...
export function HospitalInfo() {
  ...
  const addressString = [h?.addressLine1, h?.area, h?.city, h?.state, h?.pincode]
    .filter(Boolean)
    .join(", ") || "—";
    ...
    <div className="flex flex-col bg-white min-h-[calc(100vh-80px)]">
      ...
      <div className="p-4 flex flex-col gap-6">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center justify-center p-6 bg-gray-50 rounded-2xl border border-gray-100">
          ...
          <h2 className="text-2xl font-bold text-gray-900 text-center">{h?.name || "—"}</h2>
          <p className="text-sm text-gray-500 font-medium">Registration No: {h?.registrationNumber || "—"}</p>
        </motion.div>
          ...
          <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="p-4 border-b border-gray-50 flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm text-gray-600">{h?.contactPhone || "—"}</span>
              </div>
            </div>
            <div className="p-4 flex items-center gap-3">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm text-gray-600">{h?.contactEmail || "—"}</span>
              </div>
            </div>
          </div>
        </div>
